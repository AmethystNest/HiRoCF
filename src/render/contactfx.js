...
// every loadStage. Both are white so a per-particle tint can colour them.
let blobTexture = null;
let streakTexture = null;

/** Soft round puff, for dust, smoke and the impact flash. */
function makeBlobTexture() {
  const size = 32, r = size / 2;
...
/** A spark: bright at the head (right edge), fading back to nothing at
 *  the tail, and soft top to bottom so it has no hard edge. Anchored at
 *  the head so it trails backwards from the particle's position. */
 ...
 */
function makePool(texture, max, { anchorX = 0.5, anchorY = 0.5, blend = 'normal' } = {}) {
...
export function buildContactFX() {
  ...
  function reset() {
    for (const pool of [dust, smoke, sparks, flash]) {
